...
async function parseData(input) {
    ...
    const model = genAI.getGenerativeModel({ model: "gemini-pro" });
      ...
      const prompt = "Given a string extract the releavent information(name,dob,address,gender,exp)from it and convert to a json object.";
...
const validityPattern = /^\d{2}\/\d{2}\/\d{4}$/;
  ...
  try {
    ...
    //   const str = await scanGPTData(text);
      ...
      const userData = {
        ...
        validity: data.exp ? data.exp : null
      }
      if(userData.dob === null || userData.dob ===null || userData.gender === null || userData.validity === null){
        res.status(404)
      }
